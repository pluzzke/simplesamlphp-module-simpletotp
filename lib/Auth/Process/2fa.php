...
class sspmod_simpletotp_Auth_Process_2fa extends SimpleSAML\Auth\ProcessingFilter {
    /**
 ...
 not_configured_url must be a string');
            ...
            SimpleSAML\Utilities::checkURLAllowed($config['not_configured_url']);
        }
    }
    ...
    public function process(&$state) {
        ...
        if ($this->secret_val === NULL && $this->enforce_2fa === true) {
            ...
            if ($this->not_configured_url !== NULL) {
                SimpleSAML\Utilities::redirectUntrustedURL($this->not_configured_url);
            } else {
                SimpleSAML\Utilities::redirectTrustedURL(SimpleSAML\Module::getModuleURL('simpletotp/not_configured.php'));
            }
        ...
        $id  = SimpleSAML\Auth\State::saveState($state, 'simpletotp:request');
